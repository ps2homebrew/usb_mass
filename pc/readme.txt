...
PC specific:
 ...
 - hex2bin.c     // sector dump (data) conversion from text file to binary
...
function to properly initialize device). The dump function takes 2 parameters,
the start sector and the end sector to be dumped. The proces of reading
and dumping of the sectors to console window is quite slow, so be patient
(it takes more than hour). Once you have the sectors dumped in the text (hex)
form, you should convert it to binary form. For the conversion you can use
the hex2bin program.
